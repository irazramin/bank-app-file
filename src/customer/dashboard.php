                                    ...
                                    <button @click="open = !open" type="button"
                                      ...
                                      alt="Ahmed Shamim Hasan Shaon" /> -->
                                        <span
                                            ...
                                            <span
                                                class="font-medium leading-none text-emerald-700"><?= name_extract($customer->getName()) ?></span>
                                        </span>
                                    </button>
                                </div>
                            ...
                            <div class="flex-shrink-0">
                              ...
                              alt="" /> -->
                                <span
                                    class="inline-flex h-10 w-10 items-center justify-center rounded-full bg-emerald-100">
                                    <span
                                        class="font-medium leading-none text-emerald-700"><?= $_SESSION["user"]["name"] ?></span>
                                </span>
                            </div>
                            <div class="ml-3">
                                ...
                                <div class="inline-block min-w-full py-2 align-middle sm:px-6 lg:px-8">
                                    <table class="min-w-full divide-y divide-gray-300">
                                        ...
                                        <tbody class="divide-y divide-gray-200 bg-white">
                                            <?php foreach ($customer->seeTransactions($customer->getEmail()) as $transaction): ?>
                                                <?php if (($transaction["type"] == 'receive' && $transaction["receiverEmail"] == $_SESSION["user"]["email"]) || $transaction["type"] == 'receive' && $transaction["senderEmail"] == $_SESSION["user"]["email"]): ?>
                                                    <tr>
                                                        <td
                                                            class="whitespace-nowrap py-4 pl-4 pr-3 text-sm text-gray-800 sm:pl-0">
                                                            <?= $transaction["receiverEmail"] !== $_SESSION["user"]["email"] ? $transaction["receiverBy"] : ''; ?>
                                                            <?= $transaction["senderEmail"] !== $_SESSION["user"]["email"] ? $transaction["senderBy"] : ''; ?>
                                                        </td>
                                                        <td
                                                            class="whitespace-nowrap py-4 pl-4 pr-3 text-sm text-gray-500 sm:pl-0">
                                                            <?= $transaction["receiverEmail"] !== $_SESSION["user"]["email"] ? $transaction["receiverEmail"] : ''; ?>
                                                            <?= $transaction["senderEmail"] !== $_SESSION["user"]["email"] ? $transaction["senderEmail"] : ''; ?>
                                                        </td>
                                                        <?php if ($transaction["senderEmail"] == $_SESSION["user"]["email"]): ?>
                                                            <td
                                                        ...
                                                        <?php endif; ?>
                                                        <?php if ($transaction["receiverEmail"] == $_SESSION["user"]["email"]): ?>
                                                            <td

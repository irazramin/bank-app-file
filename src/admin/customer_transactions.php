...
require_once __DIR__ . '/../helpers/name_extract.php';
require_once __DIR__ . '/../helpers/formattedDate.php';
...
$customer->setName($_SESSION["user"]["name"]);
$customer->setEmail($_SESSION["user"]["email"]);
...
$userDetails = $customer->getSingleCustomer($emailParam);
?>
                ...
                <div>
                  <button @click="open = !open" type="button"
                    ...
                    <span class="inline-flex h-10 w-10 items-center justify-center rounded-full bg-sky-100">
                      <span
                        class="font-medium leading-none text-sky-700"><?= name_extract($_SESSION["user"]["name"]) ?></span>
                    </span>
                  </button>
                </div>
      ...
      <header class="py-10">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <h1 class="text-3xl font-bold tracking-tight text-white">
            Transactions of <?= $userDetails["name"] ?>
          </h1>
        </div>
      </header>
    </div>
            ...
            <div class="sm:flex sm:items-center">
              <div class="sm:flex-auto">
                <p class="mt-2 text-sm text-gray-700">
                  List of transactions made by <?= $userDetails["name"] ?>.
                </p>
              </div>
            </div>
                ...
                <div class="inline-block min-w-full py-2 align-middle sm:px-6 lg:px-8">
                  <table class="min-w-full divide-y divide-gray-300">
                    ...
                    <tbody class="divide-y divide-gray-200 bg-white">
                      <?php foreach ($customer->seeTransactions($userDetails["email"]) as $transaction): ?>
                        <?php if (($transaction["type"] == 'receive' && $transaction["receiverEmail"] == $userDetails["email"]) || $transaction["type"] == 'receive' && $transaction["senderEmail"] == $userDetails["email"]): ?>
                          <tr>
                            <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm text-gray-800 sm:pl-0">
                              <?= $transaction["receiverEmail"] !== $userDetails["email"] ? $transaction["receiverBy"] : ''; ?>
                              <?= $transaction["senderEmail"] !== $userDetails["email"] ? $transaction["senderBy"] : ''; ?>
                            </td>
                            <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm text-gray-500 sm:pl-0">
                              <?= $transaction["receiverEmail"] !== $userDetails["email"] ? $transaction["receiverEmail"] : ''; ?>
                              <?= $transaction["senderEmail"] !== $userDetails["email"] ? $transaction["senderEmail"] : ''; ?>
                            </td>
                            <?php if ($transaction["senderEmail"] == $userDetails["email"]): ?>
                              <td class="whitespace-nowrap px-2 py-4 text-sm font-medium text-red-600">
                                <?= '-$' . $transaction["amount"] ?>

                              </td>
                            <?php endif; ?>
                            <?php if ($transaction["receiverEmail"] == $userDetails["email"]): ?>
